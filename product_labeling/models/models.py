...
class Act_change_properties(models.Model):
    ...
    state = fields.Selection(
        ...
        string="Назначаемый статус",
    )
    stock_first = fields.Many2one(
        comodel_name="stock_model", string="Применить для товаров со склада"
    )
    stock_second = fields.Many2one(
        comodel_name="stock_model", string="Назначить новый склад"
    )
    product = fields.Many2one(comodel_name="product_model", string="Товар")
    count = fields.Integer(default=1, string="Количество")
    costs_income = fields.One2many(
        comodel_name="costs_income_model", inverse_name="act_id", string="Сводка"
    )
        ...
        if self.application == False:
            if self.state == "Покупка":
                ...
                for el in range(self.count):
                    record = self.env["product_lebeling_model"].create(
                        [
                            {
                                "product": self.product.id,
                                "stock": self.stock_second.id,
                                "state": self.state,
                                "costs_income": costs_income,
                            }
                        ]
                    )
                    if not costs_income:
                        costs = self.env["costs_income_model"].search(
                            [("name", "=", "Покупка"), ("act_id", "=", self.id)]
                        )
                        if not costs:
                            self.env["costs_income_model"].create(
                                [
                                    {
                                        "name": "Покупка",
                                        "act_id": self.id,
                                        "product_lebeling_id": record,
                                    }
                                ]
                            )
                        else:
            ...
            if self.state == "Перемещение":
                records = self.env["product_lebeling_model"].search(
                    [
                        ("stock", "=", self.stock_first.name),
                    ...
                    for el in range(self.count):
                        costs = self.env["costs_income_model"].search(
                            [
                                (
                                    "name",
                                    "=",
                                    f"{self.state} со склада {self.stock_first}",
                                ),
                                ("act_id", "=", self.id),
                            ]
                        )
                        if not costs:
                            self.env["costs_income_model"].create(
                                [
                                    {
                                        "name": f"{self.state} со склада {self.stock_first}",
                                        "act_id": self.id,
                                        "product_lebeling_id": records[el],
                                    }
                                ]
                            )
                        else:
                        ...
                        records[el].write(
                            {"stock": self.stock_second.id, "state": self.state}
                        )
                        records[el].costs_income += self.costs_income
                    self.application = True
                    return
                raise ValidationError(
                    "На указанном складе отсутствует указанное количество товара. Применение акта невозможно"
                )
...
class Product_lebeling(models.Model):
    ...
    code = fields.Char(
        default=generate_code, unique=True, string="Уникальный код", readonly=True
    )
    product = fields.Many2one(
        comodel_name="product_model", string="Товар", readonly=True
    )
    stock = fields.Many2one(
        comodel_name="stock_model", string="Последний назначенный склад", readonly=True
    )
    state = fields.Char(string="Последний назначенный статус", readonly=True)
    costs_income = fields.Many2many(
        comodel_name="costs_income_model", string="Сводка", readonly=True
    )
    total = fields.Float(compute="_compute_total", string="Итого")

    @api.depends()
    def _compute_total(self):
        eln = self.env["costs_income_model"].search(
            [("product_lebeling_id", "=", self.id)]
        )
        for el in eln:
            self.total += el.value
